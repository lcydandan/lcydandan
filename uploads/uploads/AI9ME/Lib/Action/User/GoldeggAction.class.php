...
class GoldeggAction extends UserAction{
	...
	public function sn(){
		...
		$record=M('Goldegg_record')->where('token="'.session('token').'" and lid='.$id.' and sn!=""')->select();
		$recordcount=M('Goldegg_record')->where('token="'.session('token').'" and lid='.$id.' and sn!=""')->count();
		$datacount=$data['firstnums']+$data['secondnums']+$data['thirdnums'];
		$this->assign('datacount',$datacount);
		$this->assign('recordcount',$recordcount);
		$this->assign('record',$record);
		$sendCount=M('Goldegg_record')->where('lid='.$id.' and sendstutas=1 and sn!=""')->count();
	...
	public function delete(){
		...
		if($back==true){
			M('Keyword')->where(array('pid'=>$id,'token'=>session('token'),'module'=>'Golegg'))->delete();
			$this->success('删除成功');
		}else{
			$this->error('操作失败');
		}
	...
	public function sendprize(){
		...
		$data['sendtime'] = time();
		$data['sendstutas'] = 1;
		$back = M('Goldegg_record')->where($where)->save($data);
		if($back==true){
			$this->success('成功发奖');
		}else{
			$this->error('操作失败');
